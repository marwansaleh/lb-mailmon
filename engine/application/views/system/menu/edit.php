            ...
            <div class="widget-content clearfix">
                <form id="MyForm" method="POST" class="form-validation">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                ...
                                <input type="number" name="sort" class="form-control" min="0" max="100" step="1" value="<?php echo $item->sort; ?>" />
                            </div>
                        </div>
                    </div>
                    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label>Give Access to Group</label>
                                <?php foreach ($groups as $group): ?>
                                <label class="fancy-checkbox">
                                    <input type="checkbox" name="access[]" value="<?php echo $group->id; ?>" <?php echo isset($item->access)&&isset($item->access[$group->id])?($item->access[$group->id]?'checked="true"':''):''; ?>>
                                    <span><?php echo $group->nama; ?></span>
                                </label>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    ...
                    <div class="form-group form-group-lg">
                        <button type="submit" class="btn btn-primary btn-large"><i class="fa fa-save"></i> Save</button>
    ...
    var Menu = {
        ...
        loadParentMenu: function(module){
            ...
            $.ajax({
                ...
                var selected_id = $select.attr('data-selected-id');
                
                if (data.status && data.item_count > 0){
                    for (var i in data.items){
                        var menu = data.items[i];
                        $select.append('<option value="'+menu.id+'">'+menu.caption+'</option>');
                    }
                }
