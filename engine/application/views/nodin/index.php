...
<script type="text/javascript">
    var Mail = {
        ...
        init: function(){
            ...
            this.dataTableObject = $('#myDataTable').DataTable({
                ...
                buttons:{
                    name: 'primary',
                    buttons: [
                        { text: '<i class="fa fa-plus"></i> New', action: function(){
                            _this.createNew();
                        }},
                ...
                serverSide: true,
                ajax: {
                    url: "<?php echo get_action_url('service/nodin/index'); ?>",
                ...
                columns:[
                    ...
                    {data: "id", class:"text-right", render: function(data,type,row){
                            ...
                            if (row.editable){
                                result += '<a class="btn btn-success btn-xs" href="<?php echo site_url('nodin/register/edit'); ?>/'+data+'" title="Edit"><i class="fa fa-pencil"></i></a>';
                            }
                            if (row.to_send){
            ...
            $('#myDataTable').on('click','.btn-view', function(){
                ...
                $modal.find('#labelDialog').text('Lihat Nota Dinas');
                $modal.find('.modal-body').load('<?php echo get_action_url('nodin/register/view'); ?>/'+item_id, function(){
                    $modal.modal('show');
                    _this.reloadDataTable();
                });
            });
            ...
            $('#myDataTable').on('click','.btn-send', function(){
                ...
                $modal.find('.modal-body').load('<?php echo get_action_url('nodin/register/approval'); ?>/'+item_id, function(){
                    $modal.modal('show');
                    _this.reloadDataTable();
                });
            });
            ...
            $('#myDataTable').on('click','.btn-sign', function(){
                ...
                $modal.find('.modal-body').load('<?php echo get_action_url('nodin/register/sign'); ?>/'+item_id, function(){
                    $modal.modal('show');
                    _this.reloadDataTable();
                });
            });
        ...
        createNew: function(){
            ...
            $modal.find('.modal-body').load('<?php echo get_action_url('nodin/register/pilihtemplate'); ?>', function(){
                $modal.modal('show');
            });
        }
    };
